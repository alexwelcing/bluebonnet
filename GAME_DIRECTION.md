...
## Canon amendment A9 (director-delegated, 2026-06-13) — SIDE B
The cassette has a second side that appears only after a viewer has finished Side A. Side B inverts the lens: the tape shows the room the deck sits in, NOW — the den lit by the CRT, bluebonnets in the carpet, the window where the street ends in flowers, the chained front door, the porch where a battery radio still carries 88.7. The timestamp burn runs LIVE (the player's real clock); the cues read NOW; the deck chrome shifts violet as the field colonizes the interface. Dana speaks in present tense through the porch radio: the tip line never closed. Two terminals: SIGN-OFF (turn the set off — containment, again) and WE ANSWER (walk into the flowers). Side B is short, dense, and never explains itself.

## Canon amendment A10 (Alex directive, 2026-06-13) — 360 WORLD BEFORE VIEWER
Do not ship a bare Gaussian-splat/model-viewer button. The current splatViewer runtime path is removed until BLUEBONNET has a real 360-degree world architecture: authored player controls, collision/proxy navigation, volumetric audio physics, spatial effect zones, accessible interaction volumes, and tape-window/day transition rules. World Labs / Marble assets may remain offline references for spatial consistency and future world packages, but they are not player-facing runtime content until promoted through gates. See `.bridge/director/360_world_architecture.md` for the current architecture brief.

## Canon amendment A11 (Alex directive, 2026-06-14) — CONTROL UI ENGINE AS SIGNATURE
The evidence deck is not website chrome and not a thin CSS skin. BLUEBONNET needs a dedicated mechanical control UI engine — Rive-like and potentially Rive-backed — so the interface becomes a strength of the game. Controls are authored as tactile state machines with pressure, release, detents, latches, faders, hard stops, refusal, audio, and accessibility feedback. Rive may become the animation backend, but puzzle logic must remain independent: the engine emits renderer-neutral control intents that can drive DOM/CSS now and Rive/canvas later. Every major deck control and performed puzzle mechanism should migrate toward this system.
