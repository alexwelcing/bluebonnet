...
describe('Act IV nine minutes content', () => {
  it('keeps Act IV clean plates in whole-shotlist A1 lint coverage', () => {
    ...
    for (const shot of act4Shots) {
      expect(shot.window).toBe('20:26-20:35');
  ...
  it('unlocks the formerly locked nine-minutes detent after recorder counter and field gate are complete', () => {
    ...
    const locked = createTimeSeek(graph, '20:17-20:26', ['20:17-20:26']);
    expect(locked.seek('20:26-20:35').ok).toBe(false);

    const unlocked = createTimeSeek(graph, '20:17-20:26', ['20:17-20:26', '20:26-20:35']);
    expect(unlocked.seek('20:26-20:35')).toMatchObject({ ok: true, activeWindow: '20:26-20:35' });

    const options = { ...defaultJogWheelOptions, discovered: ['20:08-20:17', '20:17-20:26', '20:26-20:35'] as TimeWindow[], locked: [] as TimeWindow[] };
    const nearFinal = { ...createJogWheelState('20:17-20:26', options), position: 1.99, angle: 1.99 * Math.PI * 1.5, velocity: 0.2, seatedWindow: undefined };
    expect(stepJogWheel(nearFinal, 0.1, options).event).not.toBe('hard-stop');
    expect(seatNearestDetent(nearFinal, options).state.seatedWindow).toBe('20:26-20:35');
  });
  ...
  it('gates Act IV behind field gate and recorder counter, then routes through six nine-minute nodes to the ending threshold', () => {
    ...
    for (const [nodeId, hotspotId, target] of route) {
      const node = getNodeState(graph, nodeId, '20:26-20:35');
      expect(node.caption).toContain('20:26-20:35');
  ...
  it('expands the Act IV threshold into lateral, look-down, and detail viewpoints', () => {
    const graph = loadNodeGraph([act1, act2, act3, act4] as unknown as SceneManifest[]);
    const threshold = getNodeState(graph, 'nine-field-threshold', '20:26-20:35');
    for (const target of ['nine-threshold-left', 'nine-threshold-right', 'nine-threshold-look-down', 'nine-culvert-detail']) {
      ...
      const node = getNodeState(graph, target, '20:26-20:35');
      expect(node.caption).toContain('20:26-20:35');
