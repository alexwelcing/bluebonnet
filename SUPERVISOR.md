...
- D4 MARBLE INTEGRATION: a Gaussian-splat world of the bluebonnet field is being exported to assets/worlds/. Two uses: (a) render consistent multi-angle stills FROM it so viewpoints of the same place actually match; (b) add a real explorable 3D set-piece for Act IV (the nine minutes) using a React-Three-Fiber / @mkkellogg/gaussian-splats-3d viewer behind the VHS compositor. Add a splat loader module + tests.
